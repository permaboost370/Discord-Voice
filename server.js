...
// Create an Ogg/Opus stream from PCM16k using ffmpeg-static (deferred)
...
// lazy transcoder to avoid boot-time crashes
let oggOpus = null;
function ensureTranscoder() {
  if (oggOpus) return oggOpus;
  try {
    oggOpus = pcm16kToOggOpus();
    playbackPCM16k.pipe(oggOpus);
    return oggOpus;
  } catch (e) {
    console.error('Failed to start FFmpeg (ogg/opus):', e);
    oggOpus = null;
    return null;
  }
}
...
function ensureAudioPlayer() {
  ...
  const ogg = ensureTranscoder();
  if (!ogg) {
    console.error('Cannot create audio player: transcoder not available.');
    return null;
  }
...
async function joinVoice(guild, voiceChannel) {
  ...
  if (!player) {
    console.error('Join aborted: audio player not ready (ffmpeg/transcoder issue).');
    return;
  }
  ...
  ws.on('message', (raw) => {
    try {
      ...
      if (msg.type === 'audio.delta' && msg.delta) {
        ...
        console.log('↓ audio.delta', buf.length, 'bytes');
